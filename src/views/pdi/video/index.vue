  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="文件类型" prop="mimeType">
        <el-select v-model="queryParams.mimeType" placeholder="请选择文件类型" clearable>
          <el-option
    ...
    <el-table v-loading="loading" :data="videoList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['pdi:video:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['pdi:video:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
const { pdi_upload_status, pdi_video_type, pdi_is_delete } = proxy.useDict('pdi_upload_status', 'pdi_video_type', 'pdi_is_delete');
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除管道视频编号为"' + _ids + '"的数据项？').then(function() {
...
function handleExport() {
  proxy.download('pdi/video/export', {
    ...queryParams.value
